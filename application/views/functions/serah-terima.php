<script>
  ...
  $('.btn-add').on('click', function() {
    let id_laptop          = $("select[name=id_laptop]");
    let id_karyawan        = $("select[name=id_karyawan]");
    let tgl_serah_terima   = $("input[name=tgl_serah_terima]");
    let aksesoris_1        = $("select[name=aksesoris_1]");
    let aksesoris_2        = $("select[name=aksesoris_2]");
    let aksesoris_3        = $("select[name=aksesoris_3]");

    if (!id_laptop.val().trim()) {
      iziToast.error({
        title: 'Oops!',
        message: 'Laptop tidak boleh kosong!!',
        position: 'topRight'
      });
      id_laptop.focus();
    ...
    if (!tgl_serah_terima.val()) {
      iziToast.error({
        title: 'Oops!',
        message: 'Tgl serah terima tidak boleh kosong!!',
        position: 'topRight'
      });
      tgl_serah_terima.focus();
    ...
    if (!aksesoris_1.val()) {
      iziToast.error({
        title: 'Oops!',
        message: 'Aksesoris 1 tidak boleh kosong!!',
        position: 'topRight'
      });
      aksesoris_1.focus();
    ...
    form_data.append('id_laptop', id_laptop.val());
    form_data.append('id_karyawan', id_karyawan.val());
    form_data.append('tgl_serah_terima', tgl_serah_terima.val());
    form_data.append('aksesoris_1', aksesoris_1.val());
    form_data.append('aksesoris_2', aksesoris_2.val());
    form_data.append('aksesoris_3', aksesoris_3.val());


    $.ajax({
      url: '<?php echo base_url('serah-terima/create') ?>',
      ...
      success: function(data, status) {
        if (data.status == 'success') {
          swal('Success', `${data.msg}`, 'success').then(function() {
            $(location).attr('href', `<?= base_url('serah-terima') ?>`); // redirect setelah sukses
          })
